...
#################################################### NEW WAY #######################################################
    ...
    # # Images just needed if loading a graph that was already created
    # # images = tf.map_fn(preprocess_image, jpegs, dtype=tf.float32)
    #
    # #graph = (import_inception_graph(images))
    # graph = sess.graph
    #
    # softmax_tensor = sess.graph.get_tensor_by_name('final_result:0')
    # values, indices = tf.nn.top_k(softmax_tensor, len(label_lines))
    #
    # class_descriptions = []
    # for s in range(len(label_lines)):
    #     class_descriptions.append(label_lines[s])
    ...
    #
    # signature_def = signature_def_utils.classification_signature_def(jpegs, classes, values)
...
##########################################################################################################
    ...
    # in_image = sess.graph.get_tensor_by_name('DecodeJpeg/contents:0')
    ...
    # # SAVE OUT THE MODEL
    # b = saved_model_builder.SavedModelBuilder("OUTPUT")
    ...
    print("Beginning new save method...")
    #### Newer way? ####
    ...
    optimized_output_dir = optimize_for_inference(
        FLAGS.output_graph, '/tmp/output_optimized_graph.pb', 'Mul', 'final_result')

    print("Loading frozen optimized graph for process...")
    with tf.Session() as sess:
        with gfile.FastGFile(optimized_output_dir, 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())
            tf.import_graph_def(graph_def, name='', input_map={
                                RESIZED_INPUT_TENSOR_NAME: images})
            optimized_sess = sess
            # optimized_sess.run(tf.global_variables_initializer())
            ...
            print("Setting class descriptions...")
            class_descriptions = []
            for s in range(len(label_lines)):
                class_descriptions.append(label_lines[s])
            class_tensor = tf.constant(class_descriptions)
...
def optimize_for_inference(input_dir, output_dir, input_names, output_names, placeholder_type_enum=dtypes.float32.as_datatype_enum, frozen_graph=True):
    if not gfile.Exists(input_dir):
        print("Input graph file '" + input_dir + "' does not exist!")
        return -1

    input_graph_def = graph_pb2.GraphDef()
    with gfile.Open(input_dir, "rb") as f:
        data = f.read()
        if frozen_graph:
    ...
    if frozen_graph:
        f = gfile.FastGFile(output_dir, "w")
        f.write(output_graph_def.SerializeToString())
    else:
